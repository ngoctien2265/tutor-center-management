...
function statusVi(status) { return ({ unpaid: 'Chưa thanh toán', paid: 'Đã thanh toán', active: 'Đang học', completed: 'Hoàn thành', overdue: 'Quá hạn', OPEN: 'Đang tìm gia sư', WAITING_STUDENT: 'Đang chờ học viên', TEACHING: 'Đang dạy', COMPLETED: 'Hoàn thành', CANCELLED: 'Đã hủy', APPROVED: 'Đã duyệt', PENDING: 'Chờ duyệt', REJECTED: 'Từ chối', ABSENCE_ONLY: 'Nghỉ', RESCHEDULE: 'Dạy bù', ABSENCE_WITH_MAKEUP: 'Dạy bù' }[status] || status || 'Đang học'); }
...
function ParentPortal() {
  ...
  const paidTotal = useMemo(() => paymentRows.filter((row) => row.status === 'Đã thanh toán').reduce((sum, row) => sum + Number(row.amount || 0), 0), [payments]);
  ...
  const weekSchedule = learningClasses.filter((c) => ['Đang học', 'Đang dạy'].includes(c.status)).slice(0, 4).map((c, idx) => ({ id: c.id, title: c.title, subtitle: `${c.student} - ${c.tutor}`, day: c.schedule.split(' ')[0] || ['Thứ 2', 'Thứ 3', 'Thứ 4'][idx] || 'Thứ 2', time: c.schedule.match(/\d{1,2}:\d{2}/)?.[0] || '19:00', location: c.location }));
  ...
  return <main className="tutor-figma-page parent-figma-page"><h1>{pageTitle}</h1>
    {tab === 'dashboard' && <>
      <section className="tutor-stat-row parent-stat-row parent-dashboard-stats">
        <article className="tutor-stat"><span className="tutor-icon green">▰</span><p>Lớp đang học</p><h3>{learningClasses.filter(c => c.status === 'Đang học').length}</h3></article>
        <article className="tutor-stat"><span className="tutor-icon blue">✓</span><p>Học phí đã thanh toán</p><h3>{money(paidTotal)}</h3></article>
        <article className="tutor-stat"><span className="tutor-icon purple">$</span><p>Học phí chưa thanh toán</p><h3>{money(unpaidTotal)}</h3></article>
      </section>
      {pendingConfirmations.length ? <section className="tutor-card parent-confirm-section"><div className="section-head"><div><h2>Cần phụ huynh xác nhận gia sư</h2><p className="muted">Nhân viên đã chấp thuận và gửi gia sư phù hợp. Vui lòng xác nhận để lớp chuyển sang Đang học.</p></div><button className="outline-btn compact" onClick={() => setTab('classes')}>Xem tất cả</button></div><div className="teaching-grid">{pendingConfirmations.slice(0, 2).map((item) => <ParentClassCard key={item.id} item={item} onConfirm={confirmTutor} onView={() => router.push(`/customer/classes/${item.id}`)} />)}</div></section> : null}
      <section className="tutor-dashboard-grid parent-overview-grid"><div className="tutor-card"><h2>Lịch học tuần này</h2><div className="tutor-list">{weekSchedule.length ? weekSchedule.map((item) => <div className="schedule-item" key={item.id}><div><strong>{item.title}</strong><span>{item.subtitle}</span><span>Địa chỉ: {item.location}</span></div><div className="schedule-time"><b>{item.day}</b><span>{item.time}</span></div></div>) : <p className="muted">Chưa có lịch học.</p>}</div></div><div className="tutor-card"><h2>Thông báo</h2><div className="notice-list"><div className="parent-notice blue"><strong>Nhắc nhở thanh toán</strong><span>Còn {money(unpaidTotal)} học phí cần thanh toán</span></div>{notifications.length ? notifications.slice(0, 3).map((n) => <div className="parent-notice green" key={n.requestId}><strong>{statusVi(n.requestType)} - {n.className}</strong><span>{n.sessionDate}: {n.reason}</span></div>) : <div className="parent-notice green"><strong>Trạng thái lớp</strong><span>{learningClasses.filter((c) => c.status.includes('Chờ')).length} yêu cầu đang chờ trung tâm xử lý</span></div>}</div></div></section>
    </>}
...
function ParentClassCard({ item, onView }) { const waitingStudent = item.rawStatus === 'WAITING_STUDENT' || item.status === 'Đang chờ học viên'; return <article className="teaching-card parent-class-card"><div className="class-top"><h3>{item.title}</h3><StatusBadge tone={waitingStudent ? 'yellow' : 'green'}>{item.status}</StatusBadge></div><p><b>Học viên:</b> {item.student} - {item.grade}</p><p><b>Gia sư:</b> {item.tutor}</p><p><b>Lịch:</b> {item.schedule}</p><p><b>Địa chỉ:</b> {item.location}</p><p className="green-text"><b>Học phí: {money(item.tuition)}/tháng</b></p>{item.notifications?.length ? <p className="muted">Có {item.notifications.length} thông báo nghỉ/dạy bù</p> : null}<button className="outline-btn" onClick={onView}>Xem chi tiết</button></article>; }
...
function StudentTimetable({ weekStart, setWeekStart, weekDays, sessions, onViewClass }) {
  ...
  return <section className="tutor-card timetable-detail-card">
    ...
    <div className="student-timetable-legend"><span><i className="legend-chip class" /> Lịch học</span><span><i className="legend-chip confirmed" /> Buổi đã được staff duyệt</span><span>Click vào ô lớp để xem chi tiết khóa học.</span></div>
