...
const tabKeys = ['dashboard', 'request', 'classes'];
...
function ParentPortal() {
  ...
  const pageTitle = { dashboard: 'Tổng quan', request: 'Đăng ký tìm gia sư', classes: 'Lớp đang học' }[tab];
  ...
  return <main className="tutor-figma-page parent-figma-page"><h1>{pageTitle}</h1>
    ...
    {selectedClass && <div className="modal-backdrop" onClick={() => setSelectedClass(null)}><section className="modal-card large" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setSelectedClass(null)}>×</button><h2>Chi tiết lớp học</h2><div className="detail-grid"><p><b>Môn học:</b> {selectedClass.title}</p><p><b>Học viên:</b> {selectedClass.student} - {selectedClass.grade}</p><p><b>Trường:</b> {selectedClass.school}</p><p><b>Lịch học:</b> {selectedClass.schedule}</p><p><b>Địa chỉ:</b> {selectedClass.location}</p><p><b>Học phí:</b> {money(selectedClass.tuition)}/tháng</p><p><b>Trạng thái:</b> {selectedClass.status}</p><p><b>Yêu cầu:</b> {selectedClass.requirements}</p></div>{selectedClass.tutorObj && <><hr /><h3>Thông tin gia sư</h3><div className="detail-grid"><p><b>Họ tên:</b> {selectedClass.tutorObj.fullName}</p><p><b>Đánh giá:</b> ★ {Number(selectedClass.tutorObj.rating || 0).toFixed(1)}</p><p><b>Trường:</b> {selectedClass.tutorObj.university || '-'}</p><p><b>Kinh nghiệm:</b> {selectedClass.tutorObj.bio || selectedClass.tutorObj.major || 'Chưa cập nhật'}</p><p><b>Khu vực:</b> {selectedClass.tutorObj.teachingAreas || '-'}</p><p><b>SĐT:</b> {selectedClass.tutorObj.phone || 'Chưa cập nhật'}</p><p><b>Email:</b> {selectedClass.tutorObj.email || 'Chưa cập nhật'}</p></div></>}<hr /><h3>Học phí</h3>{paymentRows.filter((row) => row.classId === selectedClass.id).length ? <div className="table-shell"><table className="tutor-table-new"><thead><tr><th>Tháng</th><th>Số tiền</th><th>Hạn thanh toán</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{paymentRows.filter((row) => row.classId === selectedClass.id).map((row) => <tr key={row.id}><td>{row.month}</td><td><b>{money(row.amount)}</b></td><td>{row.due}</td><td><StatusBadge tone={row.status === 'Đã thanh toán' ? 'green' : 'yellow'}>{row.status}</StatusBadge></td><td>{row.status !== 'Đã thanh toán' ? <button className="outline-btn compact" onClick={() => pay(row)}>Thanh toán</button> : <span className="muted">Đã thanh toán</span>}</td></tr>)}</tbody></table></div> : <p className="muted">Chưa có lịch sử học phí.</p>}<hr /><h3>Đánh giá gia sư</h3>{(() => { const current = reviewState[selectedClass.id] || { stars: 0, comment: '' }; const old = reviews.find((r) => Number(r.class_id?.id || r.classId) === Number(selectedClass.id)); return <div>{old && <p className="green-text">Đã đánh giá: {'★'.repeat(old.star_rating || old.starRating)} - {old.comment}</p>}<label>Chấm điểm:</label><div className="click-stars">{[1, 2, 3, 4, 5].map((star) => <button type="button" key={star} className={star <= current.stars ? 'active' : ''} onClick={() => setReviewState((prev) => ({ ...prev, [selectedClass.id]: { ...current, stars: star } }))}>☆</button>)}</div><label>Nhận xét:</label><textarea value={current.comment} onChange={(e) => setReviewState((prev) => ({ ...prev, [selectedClass.id]: { ...current, comment: e.target.value } }))} placeholder="Nhập nhận xét của bạn về gia sư..." /><button className="tutor-submit" onClick={() => sendReview(selectedClass.id)}>Gửi đánh giá</button></div>; })()}<hr /><h3>Thông báo nghỉ / dạy bù</h3>{selectedClass.notifications?.length ? selectedClass.notifications.map((n) => <div className="parent-notice green" key={n.requestId}><strong>{statusVi(n.requestType)} - {statusVi(n.status)}</strong><span>{n.sessionDate}: {n.reason}</span></div>) : <p className="muted">Chưa có thông báo nghỉ/dạy bù.</p>}</section></div>}
  </main>;
}
