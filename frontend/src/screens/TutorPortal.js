...
function statusVi(status) { return ({ OPEN: 'Đang tìm gia sư', WAITING_PARENT: 'Chờ phụ huynh xác nhận', WAITING_TUTOR: 'Chờ gia sư xác nhận', WAITING_STUDENT: 'Đang chờ học viên', ASSIGNED: 'Đang học', TEACHING: 'Đang học', COMPLETED: 'Hoàn thành', CANCELLED: 'Đã hủy', PENDING: 'Chờ duyệt', APPROVED: 'Đã duyệt', REJECTED: 'Từ chối', ABSENCE_ONLY: 'Nghỉ', RESCHEDULE: 'Dạy bù', ABSENCE_WITH_MAKEUP: 'Dạy bù' }[status] || status || 'Đang học'); }
...
function TutorPortal() {
  ...
  const [profile, setProfile] = useState({ fullName: '', email: '', phone: '', address: '', university: '', major: '', bio: '', teachableSubjects: '', teachableGrades: '', teachingAreas: '', bankName: '', bankBranch: '', bankAccountNumber: '', rating: 0 });
  ...
  const scheduleMatchedNotice = proposedClasses.length
    ? `Hiện có ${proposedClasses.length} lớp phù hợp với lịch rảnh của bạn. Vào mục Lớp đề xuất để nhận lớp phù hợp.`
    : 'Chưa có lớp mới khớp hoàn toàn với lịch rảnh hiện tại. Hãy cập nhật thêm khung giờ rảnh để hệ thống gợi ý chính xác hơn.';
  ...
  return <main className="tutor-figma-page"><h1>{pageTitle}</h1>
    {tab === 'dashboard' && <>
      <section className="tutor-stat-row">
        <article className="tutor-stat"><span className="tutor-icon blue">▰</span><p>Lớp đang dạy</p><h3>{dashboard.activeClasses ?? activeClasses.filter((c) => ['TEACHING','ASSIGNED','WAITING_PARENT','WAITING_STUDENT'].includes(c.status)).length}</h3></article>
      ...
      <section className="tutor-dashboard-grid dashboard-with-notice">
        <div className="tutor-card"><h2>Lịch dạy tuần này</h2><div className="tutor-list">{weekSchedule.length ? weekSchedule.map((item, idx) => <div className="schedule-item" key={idx}><div><strong>{item.subject}</strong><span>{item.studentName || 'Học viên'}</span><span>Địa chỉ: {item.location || item.address || 'Chưa cập nhật'}</span></div><div className="schedule-time"><b>{item.dayLabel || item.schedule?.[0]?.dayLabel || 'Thứ'}</b><span>{item.startTime || item.schedule?.[0]?.startTime || ''}</span></div></div>) : <p className="muted">Chưa có lịch dạy.</p>}</div></div>
        <div className="tutor-card tutor-dashboard-notice"><h2>Thông báo lịch rảnh</h2><div className="schedule-match-box"><strong>{proposedClasses.length ? 'Có lớp phù hợp' : 'Chưa có lớp phù hợp'}</strong><p>{scheduleMatchedNotice}</p><button className="outline-btn compact" onClick={() => setTab(proposedClasses.length ? 'proposed' : 'timetable')}>{proposedClasses.length ? 'Xem lớp đề xuất' : 'Cập nhật lịch rảnh'}</button></div><div className="mini-dashboard-list"><span>Lớp đang chờ ứng tuyển: <b>{dashboard.pendingApplications || 0}</b></span><span>Yêu cầu nghỉ/bù chờ duyệt: <b>{(dashboard.pendingAbsenceRequests || 0) + (dashboard.pendingMakeupRequests || 0)}</b></span></div></div>
      </section>
    </>}
    ...
    {tab === 'profile' && <form className="profile-stack beautiful-profile" onSubmit={(e) => { e.preventDefault(); saveProfile(); }}><section className="tutor-card profile-hero"><div className="profile-avatar">{(profile.fullName || 'GS').slice(0, 2).toUpperCase()}</div><div><h2>{profile.fullName || 'Gia sư'}</h2><p>{profile.university || 'Cập nhật trường học'} • {profile.major || 'Cập nhật chuyên ngành'}</p><span className="rating">★ {avgRating}</span></div></section><section className="tutor-card profile-card"><h2>Thông tin cá nhân</h2><div className="profile-grid two"><label>Họ và tên<input value={profile.fullName || ''} onChange={(e) => setProfile({ ...profile, fullName: e.target.value })} /></label><label>Email<input value={profile.email || ''} readOnly /></label><label>Số điện thoại<input value={profile.phone || ''} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} /></label><label>Địa chỉ<input value={profile.address || ''} onChange={(e) => setProfile({ ...profile, address: e.target.value })} /></label><label>Trường đại học<input value={profile.university || ''} onChange={(e) => setProfile({ ...profile, university: e.target.value })} /></label><label>Chuyên ngành<input value={profile.major || ''} onChange={(e) => setProfile({ ...profile, major: e.target.value })} /></label></div><label>Kinh nghiệm<textarea className="profile-textarea" value={profile.bio || ''} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} /></label></section><section className="tutor-card profile-card"><h2>Năng lực giảng dạy</h2><div className="profile-grid two"><label>Môn có thể dạy<input value={profile.teachableSubjects || ''} onChange={(e) => setProfile({ ...profile, teachableSubjects: e.target.value })} /></label><label>Khối lớp<input value={profile.teachableGrades || ''} onChange={(e) => setProfile({ ...profile, teachableGrades: e.target.value })} /></label></div><label>Khu vực dạy<input value={profile.teachingAreas || ''} onChange={(e) => setProfile({ ...profile, teachingAreas: e.target.value })} /></label></section><section className="tutor-card profile-card"><h2>Thông tin tài khoản ngân hàng</h2><div className="profile-grid two"><label>Tên ngân hàng<input value={profile.bankName || ''} onChange={(e) => setProfile({ ...profile, bankName: e.target.value })} placeholder="VD: Vietcombank" /></label><label>Chi nhánh<input value={profile.bankBranch || ''} onChange={(e) => setProfile({ ...profile, bankBranch: e.target.value })} placeholder="VD: Chi nhánh Quận 1" /></label><label>Số tài khoản ngân hàng<input value={profile.bankAccountNumber || ''} onChange={(e) => setProfile({ ...profile, bankAccountNumber: e.target.value })} placeholder="VD: 0123456789" /></label></div></section><button className="tutor-submit full-width">Lưu hồ sơ</button></form>}

    {tab === 'classes' && <>
      <section className="tutor-card"><h2>Lớp đang dạy</h2><div className="teaching-grid tutor-class-management">{activeClasses.length ? activeClasses.map((cls) => <article className="teaching-card" key={cls.classId}><div className="class-top"><h3>{cls.subject} - {cls.level}</h3><StatusBadge tone={['WAITING_PARENT', 'WAITING_STUDENT'].includes(cls.status) ? 'yellow' : 'green'}>{statusVi(cls.status)}</StatusBadge></div><p><strong>Học viên:</strong> {cls.studentName}</p><p><strong>SĐT học viên:</strong> {cls.studentPhone || 'Chưa cập nhật'}</p><p><strong>Lịch:</strong> {scheduleLabel(cls.schedule)}</p><p><strong>Địa chỉ:</strong> {cls.location}</p><p className="green-text"><b>Lương: {money(Number(cls.salaryPerSession || 0) * Number(cls.sessionsPerWeek || 2) * 4)}/tháng</b></p><div className="suggest-actions wrap"><button className="outline-btn compact" onClick={() => startAction(cls, 'ABSENCE')}>Xin nghỉ</button><button className="outline-btn compact" onClick={() => startAction(cls, 'MAKEUP')}>Dạy bù</button></div></article>) : <p className="muted">Chưa có lớp đang dạy.</p>}</div></section>
      {actionForm && <form className="tutor-card action-panel polished-action-panel" onSubmit={submitClassAction}>
        <div className="action-panel-head"><div><span>{actionForm.type === 'ABSENCE' ? 'Yêu cầu nghỉ' : 'Yêu cầu dạy bù'}</span><h2>{actionForm.className}</h2><p className="muted">Thông tin sẽ được gửi cho nhân viên duyệt trước khi cập nhật vào lịch học.</p></div><button type="button" className="tutor-muted-btn compact" onClick={() => setActionForm(null)}>Đóng</button></div>
        <div className="action-form-grid"><label>Ngày cần xử lý<input type="date" value={actionForm.date} onChange={(e) => setActionForm({ ...actionForm, date: e.target.value })} /></label><label className="action-note-field">Lý do / ghi chú<textarea className="profile-textarea" value={actionForm.note} onChange={(e) => setActionForm({ ...actionForm, note: e.target.value })} placeholder="VD: Em bận lịch thi nên xin nghỉ buổi này, hoặc đề xuất dạy bù vào tối hôm sau..." /></label></div>
        <div className="action-panel-footer"><p className="muted">Nhân viên sẽ kiểm tra lịch lớp và phản hồi trạng thái duyệt.</p><button className="tutor-primary">Gửi yêu cầu</button></div>
      </form>}
    </>}
