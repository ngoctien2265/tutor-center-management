...
function statusVi(status) { return ({ OPEN: 'Đang tìm gia sư', WAITING_STUDENT: 'Đang chờ học viên', TEACHING: 'Đang dạy', COMPLETED: 'Hoàn thành', CANCELLED: 'Đã hủy', PENDING: 'Chờ duyệt', APPROVED: 'Đã duyệt', REJECTED: 'Từ chối', ABSENCE_ONLY: 'Nghỉ', RESCHEDULE: 'Dạy bù', ABSENCE_WITH_MAKEUP: 'Dạy bù' }[status] || status || 'Đang học'); }
  ...
  return <main className="tutor-figma-page"><h1>{pageTitle}</h1>
    {tab === 'dashboard' && <>
      <section className="tutor-stat-row">
        <article className="tutor-stat"><span className="tutor-icon blue">▰</span><p>Lớp đang dạy</p><h3>{dashboard.activeClasses ?? activeClasses.filter((c) => ['TEACHING','WAITING_STUDENT'].includes(c.status)).length}</h3></article>
