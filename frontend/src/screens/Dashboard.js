...
function LineChart({ values = [], labels = [], color = '#3b82f6', unit = '' }) {
  const max = niceMax(values, 10);
  const ticks = [0, max * 0.25, max * 0.5, max * 0.75, max];
  const safeLabels = labels.length ? labels : ['T1', 'T2', 'T3', 'T4', 'T5', 'T6'];
  const safeValues = safeLabels.map((_, i) => Number(values[i] || 0));
  const points = safeValues.map((v, i) => `${70 + i * 104},${282 - (v / max) * 232}`).join(' ');
  return (
    <svg className="chart-svg" viewBox="0 0 620 340" role="img">
      {[0, 1, 2, 3, 4].map((i) => <line key={i} x1="70" x2="590" y1={50 + i * 58} y2={50 + i * 58} stroke="#d1d5db" strokeDasharray="4 4" />)}
      {safeLabels.map((_, i) => <line key={i} x1={70 + i * 104} x2={70 + i * 104} y1="50" y2="282" stroke="#d1d5db" strokeDasharray="4 4" />)}
      <line x1="70" x2="590" y1="282" y2="282" stroke="#6b7280" />
      <line x1="70" x2="70" y1="50" y2="282" stroke="#6b7280" />
      {ticks.map((v, i) => <text key={v} x="58" y={288 - i * 58} textAnchor="end" fill="#64748b" fontSize="15">{Math.round(v)}{unit}</text>)}
      <polyline fill="none" stroke={color} strokeWidth="3" points={points} />
      {safeValues.map((v, i) => <circle key={i} cx={70 + i * 104} cy={282 - (v / max) * 232} r="4" fill="#fff" stroke={color} strokeWidth="3" />)}
      {safeLabels.map((m, i) => <text key={m} x={70 + i * 104} y="310" textAnchor="middle" fill="#64748b" fontSize="17">{m}</text>)}
    </svg>
  );
}
