  ...
  useEffect(() => {
    axios.get('/v1/admin/finance/summary')
      .then((res) => {
        ...
        setSalaries(res.data.data?.salaryRows || []);
  ...
  return (
    <main className="dashboard-container">
      ...
      <section className="stat-row">
        <article className="admin-stat money-card"><span className="icon green">$</span><p>Doanh thu tháng này</p><h3>{money(revenue)}</h3><small>Đồng bộ từ học phí đã thanh toán</small></article>
        <article className="admin-stat money-card"><span className="icon blue">▤</span><p>Lương gia sư</p><h3>{money(salary)}</h3><span className="muted">Tổng lương dự kiến trong tháng</span></article>
        <article className="admin-stat money-card"><span className="icon purple">↗</span><p>Lợi nhuận</p><h3>{money(profit)}</h3><span className="muted">Doanh thu - lương gia sư</span></article>
        <article className="admin-stat money-card"><span className="icon orange">▭</span><p>Học phí chưa thu</p><h3>{money(uncollected)}</h3><span className="muted">Các khoản chưa thanh toán</span></article>
      </section>
      <section className="panel-grid-2">
        <article className="chart-card"><h2>Doanh thu & chi phí của hệ thống (Triệu VNĐ)</h2><BarChart labels={charts.labels} revenue={charts.revenue} salary={charts.salary} /></article>
        <article className="chart-card"><h2>Lợi nhuận của hệ thống (Triệu VNĐ)</h2><LineChart labels={charts.labels} values={charts.profit} /></article>
        <article className="table-card"><h2>Lịch sử thanh toán học phí gần đây</h2><table className="admin-table compact-table"><thead><tr><th>Phụ huynh</th><th>Lớp học</th><th>Số tiền</th><th>Ngày</th><th>Trạng thái</th></tr></thead><tbody>{payments.map((p)=><tr key={p.id || p.parent+p.className+p.date}><td><strong>{p.parent}</strong></td><td>{p.className}</td><td><strong>{money(p.amount)}</strong></td><td>{p.date}</td><td><span className={`status-badge ${statusClass(p.status)}`}>{statusLabel(p.status)}</span></td></tr>)}</tbody></table></article>
        <article className="table-card"><h2>Bảng lương gia sư tháng này</h2><table className="admin-table compact-table"><thead><tr><th>Gia sư</th><th>Lớp</th><th>Buổi dạy</th><th>Lương</th><th>Trạng thái</th></tr></thead><tbody>{salaries.map((s)=><tr key={s.tutorId || s.tutorName}><td><strong>{s.tutorName}</strong></td><td>{s.classes}</td><td>{s.sessionLabel || `${s.sessions || 0} buổi trong tháng`}</td><td><strong>{money(s.salary)}</strong></td><td><span className={`status-badge ${statusClass(s.status)}`}>{statusLabel(s.status)}</span></td></tr>)}</tbody></table></article>
      </section>
    </main>
  );
}
